...
from Dataloader.SeAct_sampled_dataset import SeAct_sampled, event_sampled_frames_collate_func
...
#------------------------------------------------------ev -> text----------------------------------------------
    for events, actual_event_length, labels in dataloader:
        ...
        with torch.no_grad():
            ...
            if logit_scale.ndim != 0:
                logit_scale = logit_scale[0]
            # print(txt_embeds_.shape)
            txt_embeds_, txt_f = txt_embeds_[:58,:], txt_f[:58,:]
            logits = logit_scale * ev_embeds_ @ txt_embeds_.t()
...
def train_Conceptualizing_Alignment(HumanECLIP, cfg):
    ...
    print('----------------------------------------------------')
    for name, param in HumanECLIP.named_parameters():
    ...
    return HumanECLIP, cfg

def train_EventEncoder(HumanECLIP, cfg):
...
def train_Conceptualizing_Alignment_EventEncoder(HumanECLIP, cfg):
    cfg['LossFunction']['loss_cross_entroy'] = True
    cfg['LossFunction']['loss_cross_entroy_ori'] = False
    cfg['LossFunction']['loss_logsigma_sum'] = True
    cfg['LossFunction']['loss_SmoothL1Loss_ev'] = False
...
if __name__ == '__main__':
    # ---------------------------------------------------init----------------------------------------------------------------
    cfg = read_yaml('./Configs/SeAct.yaml')
    ...
    train_loader = DataLoader(train_dataset, batch_size=cfg['Dataset']['Train']['Batch_size'],
                              shuffle=True, drop_last=True, num_workers=16, prefetch_factor=2, pin_memory=True,
    ...
    if cfg['MODEL']['Load_Path'] != None:
        ExACT.load_state_dict(torch.load(cfg['MODEL']['Load_Path']))
    # ----------------------------------------------optimizer&lr-------------------------------------------------------------
    ...
    ExACT_original, cfg = train_EventEncoder(ExACT, cfg)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if acc1_ev >= best_acc1_ev:
            ...
            torch.save(ExACT_original.state_dict(), PATH)
