...
from model.utils.plot import plot_tsne
...
#------------------------------------------------------ev -> text----------------------------------------------
    for events, actual_event_length, labels in dataloader:
        ...
        with torch.no_grad():
            ...
            if logit_scale.ndim != 0:
                logit_scale = logit_scale[0]
            txt_embeds_, txt_f = txt_embeds_[:300,:], txt_f[:300,:]
            logits = logit_scale * ev_embeds_ @ txt_embeds_.t()
        ...
        for i in range(B):
            ...
            if scores_ev_i.topk(1)[1].cpu().detach().numpy()[0]+1 == label_i:
                hit1_ev += 1

            if int(label_i) in set(scores_ev_i.topk(5)[1].cpu().detach().numpy()+1):
                hit5_ev += 1
            ...
            if scores_ev_ori_i.topk(1)[1].cpu().detach().numpy()[0]+1 == label_i:
                hit1_ev_ori += 1
            if int(label_i) in set(scores_ev_ori_i.topk(5)[1].cpu().detach().numpy()+1):
                hit5_ev_ori += 1
...
if __name__ == '__main__':
    # ---------------------------------------------------init----------------------------------------------------------------
    cfg = read_yaml('./Configs/HARDVS.yaml')
    ...
    lr_sched = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=cfg['Trainer']['epoch'],
